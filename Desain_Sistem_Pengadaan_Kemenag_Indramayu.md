...
| **3** | Belum mulai | Pagu, Jenis Pengadaan, Paket | Master JENIS_PENGADAAN, PAGU, CRUD PAKET, `computePaketCompletion()` awal (tanpa dokumen dulu) | Paket bisa dibuat & dihitung kelengkapan datanya |
| **4** | Belum mulai | Penyedia | CRUD PROVIDERS, upload & versi Company Profile, pilih penyedia existing di paket | Satu penyedia bisa dipakai di ≥2 paket tanpa upload ulang |
| **5** | Belum mulai | Document Management + Drive | MASTER_DOCUMENT_REQUIREMENTS, upload/replace/hapus (soft-delete), checklist, `getOrCreateFolder()` | Checklist dokumen akurat, versi lama tidak hilang |
| **6** | Belum mulai | KAK Generator | TEMPLATES, `generateKak`/`finalizeKak`, deteksi kadaluarsa snapshot | KAK ter-generate dari data asli paket, bisa diregenerasi |
| **7** | Belum mulai | HPS Spreadsheet | Grid native, paste handler (text/html), import SheetJS, `generateHps` | Copy-paste dari Excel masuk dengan total benar |
| **8** | Belum mulai | Reporting + Monitoring lanjutan | Rekap per satker/user/jenis/tahun, export Excel/CSV/PDF, notifikasi, pencarian global | Rekap cocok dengan data mentah |
| **9** | Belum mulai | Security hardening | CAPTCHA (jika dipilih), rate-limit, review authorization di semua endpoint, uji kasus AUTH-001/002 | Semua test case keamanan di daftar Anda lolos |
| **10** | Belum mulai | Testing + Deployment | Jalankan seluruh test case (LOGIN-*, AUTH-*, PKT-*, DOC-*, HPS-*, KAK-*, PROV-*, COMP-*), deployment produksi | Checklist test case 100% lolos, Web App di-deploy sebagai versi stabil |

Kode Tahap 1+2 ada di `pengadaan-tahap1-2-github-pages.zip` (backend Apps Script + frontend GitHub Pages terpisah, lihat Bagian A).
